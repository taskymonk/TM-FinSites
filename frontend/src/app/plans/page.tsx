...
export default function PlansPage() {
  ...
  return (
    <div className="min-h-screen bg-[#030712]" data-testid="plans-page">
      <Navbar />
      <div className="pt-20 pb-16">
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="text-center mb-14">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-slate-800/60 bg-slate-900/30 text-xs font-medium text-slate-400 mb-4">
              <Zap className="w-3.5 h-3.5 text-blue-400" /> Transparent Pricing
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
              {plans.map((plan, i) => (
                <motion.div key={plan.plan_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                  <Card className={`relative h-full bg-white/[0.03] border border-white/[0.06] rounded-2xl ${plan.is_popular ? "ring-2 ring-primary glow-primary" : ""} hover:-translate-y-1 transition-all duration-300`} data-testid={`plan-card-${plan.plan_id}`}>
                    {plan.is_popular && (
                      <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-blue-600 text-blue-400-foreground text-xs font-semibold">
                        Most Popular
                      </div>
                    )}
                    <CardHeader className="pb-2">
                      <CardTitle className="text-xl font-[family-name:var(--font-heading)] text-white">{plan.name}</CardTitle>
                    ...
                    <CardContent className="flex flex-col h-full">
                      ...
                      <Button onClick={() => selectPlan(plan)} variant={plan.is_popular ? "default" : "outline"}
                        className="w-full font-semibold bg-blue-600 hover:bg-blue-500 text-white" data-testid={`plan-select-${plan.plan_id}`}>
          ...
          <div className="mt-16 text-center p-8 rounded-2xl glass max-w-2xl mx-auto">
            ...
            <p className="text-sm text-slate-400 mb-4">Run a free compliance audit on your current website first.</p>
      ...
      <Dialog open={showEnterprise} onOpenChange={setShowEnterprise}>
        <DialogContent className="sm:max-w-md" data-testid="enterprise-dialog">
          ...
          <form onSubmit={submitEnterprise} className="space-y-4">
            ...
            <Button type="submit" className="w-full font-semibold bg-blue-600 hover:bg-blue-500 text-white" disabled={submitting} data-testid="enterprise-submit">
