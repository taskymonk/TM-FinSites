...
function detectBusinessTypes(text: string, $: cheerio.CheerioAPI): string[] {
  ...
  // Also extract text from link labels, alt attributes, and data attributes for better coverage
  const linkTexts = $("a").map((_, el) => $(el).text().toLowerCase()).get().join(" ")
  const imgAlts = $("img").map((_, el) => ($(el).attr("alt") || "").toLowerCase()).get().join(" ")
  const allText = lowerText + " " + linkTexts + " " + imgAlts

  const checks: [string, RegExp[]][] = [
    ["MFD", [/mutual fund/i, /amfi/i, /arn[:\s-]*\d/i, /\bmfd\b/i, /mutual fund distributor/i, /amfi[\s-]*registered/i, /fund\s*management/i]],
    ["Insurance", [/irdai/i, /\binsurance\b/i, /\blic\b/i, /\bposp\b/i, /insurance broker/i, /life\s*insurance/i, /health\s*insurance/i, /general\s*insurance/i, /insurance\s*planning/i]],
    ["RIA", [/investment advis/i, /\bria\b/i, /sebi.*advis/i, /registered investment/i, /\badvisory\b/i, /fee[\s-]*based.*advis/i]],
    ["PMS", [/portfolio management/i, /\bpms\b/i, /discretionary/i, /non-discretionary/i]],
    ["Stock Broker", [/stock\s*brok/i, /\btrading\b/i, /\bnse\b/i, /\bbse\b/i, /authorized\s*person/i, /sub[\s-]?broker/i, /demat/i]],
    ["SIF", [/specialised investment fund/i, /specialized investment fund/i, /\bsif\b/i, /nism.*xxi/i]],
    ["NPS", [/\bnps\b/i, /national pension/i, /pfrda/i, /pension\s*fund/i]],
  ]
  for (const [type, patterns] of checks) {
    const matches = patterns.filter((p) => p.test(allText)).length
    if (matches >= 1) detected.push(type)
  }
  if (detected.length === 0) {
    if (/financ/i.test(allText) || /invest/i.test(allText)) detected.push("MFD")
  }
  return detected
}
...
const COMPLIANCE_RULES: ComplianceRule[] = [
  ...
  },
  {
    ...
    check: (ctx) => {
      ...
      const hasAmfiReg = /AMFI[\s-]*(registered|reg)/i.test(ctx.text)
      const passed = hasArn || hasAmfiReg
      return { passed, details: hasArn ? "AMFI ARN number found" : hasAmfiReg ? "AMFI-registered status found (ARN number recommended)" : "AMFI ARN number not displayed on website" }
    },
  },
  {
  ...
  // CONTACT & GRIEVANCE
  {
    ...
    check: (ctx) => {
      ...
      const hasContactPage = ctx.links.some((l) => /contact/i.test(l)) || ctx.$('a').filter((_, el) => /contact/i.test(ctx.$(el).text())).length > 0
      const passed = hasEmail || hasPhone || hasContact || hasContactPage
  ...
  },
  {
    ...
    check: (ctx) => {
      ...
      const hasDesc = (ctx.$('meta[name="description"]').attr("content")?.length ?? 0) > 10 || (ctx.meta["description"]?.length ?? 0) > 10
  ...
  // CONTENT QUALITY
  {
    ...
    businessTypes: ["MFD", "Insurance", "RIA", "PMS", "Stock Broker", "SIF", "NPS"],
    check: (ctx) => {
      const has = /about\s*(us|me|the\s*firm|the\s*company)/i.test(ctx.text) || ctx.$('a').filter((_, el) => /about/i.test(ctx.$(el).text())).length > 0 || ctx.links.some((l) => /about|companyprofile|who[\s-]*we[\s-]*are/i.test(l))
  ...
  },
  {
    ...
    businessTypes: ["MFD", "Insurance", "RIA", "PMS", "Stock Broker", "SIF", "NPS"],
    check: (ctx) => {
      const has = /service/i.test(ctx.text) || /what we (do|offer)/i.test(ctx.text) || /our\s*(services|offerings|products)/i.test(ctx.text) || ctx.$('a').filter((_, el) => /service|product/i.test(ctx.$(el).text())).length > 0 || ctx.links.some((l) => /service|product/i.test(l))
  ...
  { id: "REG-008", name: "Registered Office Address", category: "Registration & Identity", severity: "major", businessTypes: ["MFD", "Insurance", "RIA", "PMS", "Stock Broker", "SIF", "NPS"],
    check: (ctx) => { const has = /registered\s*(office|address)/i.test(ctx.text) || /office\s*address/i.test(ctx.text) || /address\s*:/i.test(ctx.text) || /\d{6}/.test(ctx.text) || /(nagar|colony|sector|floor|road|street|tower|block|lane)/i.test(ctx.text); return { passed: has, details: has ? "Office address found" : "Registered office address not displayed" } } },
  ...
  { id: "DISC-010", name: "KYC Requirement Mention", category: "Disclaimers & Disclosures", severity: "major", businessTypes: ["MFD", "RIA", "PMS", "Stock Broker"],
    check: (ctx) => { const has = /KYC/i.test(ctx.text) || /know\s*your\s*(customer|client)/i.test(ctx.text) || ctx.links.some((l) => /kyc/i.test(l)) || ctx.$('a').filter((_, el) => /kyc/i.test(ctx.$(el).text())).length > 0; return { passed: has, details: has ? "KYC requirement mentioned" : "KYC requirements not mentioned — mandatory for all SEBI-regulated entities" } } },
  ...
  { id: "QUAL-005", name: "Contact Form / CTA", category: "Content Quality", severity: "minor", businessTypes: ["MFD", "Insurance", "RIA", "PMS", "Stock Broker", "SIF", "NPS"],
    check: (ctx) => { const has = ctx.$("form").length > 0 || ctx.$('a[href*="contact"]').length > 0 || ctx.$('button').filter((_, el) => /contact|enquir|get in touch|reach/i.test(ctx.$(el).text())).length > 0 || ctx.$('a').filter((_, el) => /enquir|contact\s*us|get\s*in\s*touch/i.test(ctx.$(el).text())).length > 0 || ctx.links.some((l) => /contact/i.test(l)); return { passed: has, details: has ? "Contact form or CTA found" : "No contact form or call-to-action button found" } } },
  ...
  { id: "QUAL-007", name: "Blog / Knowledge Section", category: "Content Quality", severity: "minor", businessTypes: ["MFD", "Insurance", "RIA", "PMS", "Stock Broker"],
    check: (ctx) => { const has = /blog/i.test(ctx.text) || /articles/i.test(ctx.text) || /knowledge/i.test(ctx.text) || /insights/i.test(ctx.text) || ctx.$('a').filter((_, el) => /blog/i.test(ctx.$(el).text())).length > 0 || ctx.links.some((l) => /blog|article/i.test(l)); return { passed: has, details: has ? "Blog/knowledge section found" : "No blog or knowledge section — good for SEO and credibility" } } },
  ...
  { id: "QUAL-009", name: "Calculator / Tool", category: "Content Quality", severity: "minor", businessTypes: ["MFD", "Insurance", "RIA"],
    check: (ctx) => { const has = /calculator/i.test(ctx.text) || /SIP\s*calculator/i.test(ctx.text) || /retirement\s*calculator/i.test(ctx.text) || /EMI/i.test(ctx.text) || ctx.links.some((l) => /calculator/i.test(l)) || ctx.$('a').filter((_, el) => /calculator/i.test(ctx.$(el).text())).length > 0; return { passed: has, details: has ? "Financial calculator/tool found" : "No financial calculator — SIP/retirement/goal calculators improve engagement" } } },
  ...
  { id: "SEO-003", name: "Meta Description Length", category: "SEO & Performance", severity: "minor", businessTypes: ["MFD", "Insurance", "RIA", "PMS", "Stock Broker", "SIF", "NPS"],
    check: (ctx) => { const desc = ctx.meta["description"] || ctx.meta["og:description"] || ""; return { passed: desc.length >= 50 && desc.length <= 160, details: desc.length === 0 ? "No meta description" : `Description is ${desc.length} chars — ${desc.length < 50 ? "too short" : desc.length > 160 ? "too long" : "good length"}` } } },
  ...
  { id: "CRO-001", name: "Clear Call-to-Action", category: "Conversion & UX", severity: "minor", businessTypes: ["MFD", "Insurance", "RIA", "PMS", "Stock Broker", "SIF", "NPS"],
    check: (ctx) => { const ctas = ctx.$("a, button").filter((_, el) => /(contact|enquir|get.*start|book.*call|schedule|consult|invest\s*now|start\s*sip|open.*account|read\s*more|client\s*login|need\s*expert)/i.test(ctx.$(el).text())).length; return { passed: ctas >= 1, details: ctas >= 1 ? `${ctas} clear CTAs found` : "No clear call-to-action buttons" } } },
...
export function runAudit(url: string, html: string): AuditResult {
  ...
  // Enhanced text extraction: body text + link labels + alt attributes + title attributes + data attributes
  const bodyText = $("body").text().replace(/\s+/g, " ").trim()
  const linkLabels = $("a").map((_, el) => $(el).text()).get().join(" ")
  const altTexts = $("img").map((_, el) => $(el).attr("alt") || "").get().join(" ")
  const titleAttrs = $("[title]").map((_, el) => $(el).attr("title") || "").get().join(" ")
  const dataMessages = $("[data-message]").map((_, el) => $(el).attr("data-message") || "").get().join(" ")
  const text = [bodyText, linkLabels, altTexts, titleAttrs, dataMessages].join(" ").replace(/\s+/g, " ").trim()
